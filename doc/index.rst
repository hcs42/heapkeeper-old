...
* *License*: Heapkeeper is a free software. It is distributed under the `GNU
  General Public License version 3`_.
* *Download*: A released version of Heapkeeper can be downloaded from the
  `download page`_.
* *Installation*: Heapkeeper requires Python_ 2.5 or 2.6 installed. After
  downloading, Heapkeeper can be executed without any specific installation
  steps.
* *Hosting*: The source code of the program and the documentation is stored in
  the `Heapkeeper repository`_, which is hosted by GitHub_, and can be
  used with the Git_ version control system.
* *Environment*: Heapkeeper is written in Python_; more precisely, it
  runs with Python 2.5 or 2.6. The documentation is written in
  reStructuredText_ format and is generated by Sphinx_. Heapkeeper
